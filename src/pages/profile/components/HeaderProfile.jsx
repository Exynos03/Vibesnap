...
const HeaderProfile = ({
  ...
  const handleFileChange = (event, inputType) => {
    ...
    if (files && files.length > 0) {
      toast.promise(uploadPhotoUpload(inputType, files[0]), {
        loading: "Updating...",
        success:
          inputType === "profile"
  ...
  return (
    <header>
      ...
      <div className={styles.cover_photo}>
        ...
        {editMode && (
          <div
            className={styles.cover_edit_btn_div}
            onClick={() => handleFileInputClick(coverInputRef)}
          >
            <MdEdit className={styles.edit_btn} />
          </div>
        )}
      </div>
      {/* Profile photo */}
      <div className={styles.profile_photo}>
        ...
        {editMode && (
          <div
            className={styles.edit_btn_div}
            onClick={() => handleFileInputClick(profileInputRef)}
          >
            <MdEdit className={styles.edit_btn} />
          </div>
        )}
      ...
      {!editMode && location.pathname === "/profile" && (
