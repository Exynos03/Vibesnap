...
const CreatePost = () => {
  ...
  const uploadFilesToFirebase = async () => {
    ...
    try {
      const uploadPromises = files.map((file) => uploadMedia(file, folderName));
      ...
      <FaArrowLeft
        ...
        style={
          window.innerWidth > 768
            ? { marginLeft: "5%", marginTop: "4%", cursor: "Pointer" }
            : { marginLeft: "8%", marginTop: "5%", cursor: "Pointer" }
        }
        onClick={() => navigate(-1)}
      />
      <MediaViewer
